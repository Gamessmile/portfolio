document.addEventListener('DOMContentLoaded', () => {
    ...
    const isAbsoluteUrl = (url) => /^(https?:|data:|mailto:|tel:|#|\/\/)/i.test(url);
    ...
    const rewriteReadmeHtml = (html, repo, branch) => {
        ...
        wrapper.querySelectorAll('a[href]').forEach(a => {
            ...
            if (!href) return;
            a.setAttribute('href', resolveUrl(href, blobBase));
            a.target = '_blank';
            a.rel = 'noopener noreferrer';
        });

        wrapper.querySelectorAll('img[src], source[src], image[href], image[xlink\\:href], use[href], use[xlink\\:href]').forEach(el => {
            const src = el.getAttribute('src') || el.getAttribute('href') || el.getAttribute('xlink:href');
            if (!src) return;
            const fixed = resolveUrl(src, rawBase);
            if (el.hasAttribute('src')) el.setAttribute('src', fixed);
            if (el.hasAttribute('href')) el.setAttribute('href', fixed);
            if (el.hasAttribute('xlink:href')) el.setAttribute('xlink:href', fixed);
        });

        wrapper.querySelectorAll('[srcset]').forEach(el => {
            ...
            const fixedSrcset = srcset
                ...
                .map(part => {
                    ...
                    const fixedUrl = resolveUrl(url, rawBase);
                    return [fixedUrl, ...rest].join(' ');
                })
                .join(', ');
        ...
        .then(config => {
            ...
            if (skillsContainer && config.skills) {
                ...
                Object.entries(config.skills).forEach(([category, skills], idx) => {
                    ...
                    else if (category.includes('Lingue')) catKey = 'cat_languages';
        ...
        const openModal = (card) => {
            ...
            fetch(`https://api.github.com/repos/${repo}/readme`, { headers: { Accept: 'application/vnd.github.html' } })
                ...
                .then(html => {
                    ...
                    body.innerHTML = rewriteReadmeHtml(cleanHTML, repo, branch);
                })
                .catch(() => {
